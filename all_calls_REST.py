...
def post_hierarchy():
    ...
    if nonRoot:
        qryStr = "MATCH p=(fParent:Feature)-[:PARENT_OF]->(f:Feature {id:'" + in_params_nodeId.replace("\"", "") + "'})-[*]->(f2:Feature {depth: '"+ depth + "'}) RETURN f AS root, COLLECT(f2) AS leaf, nodes(p) AS path, fParent"
    else:
    ...
    for j in range(0, len(paths)):
        ...
        if  depth_2_child_list is not None:
            ...
            if not in_iter:
                ...
                depth_1_children_ret[depth_1_children_index[depth_1_parent_id]]['children'][child_index]['children'].append(returnChildren(paths[j][3]))
                print("appened child for " + paths[j][3]['parentId'])
                depth_3_children[paths[j][3]['id']] = len(depth_1_children_ret[depth_1_children_index[depth_1_parent_id]]['children'][child_index]['children'])-1
        else:
            if parent_id in depth_2_children.keys():
                print("key present")
            depth_1_children[depth_1_parent_id][0]['children'][z]['children'] = [returnChildren(paths[j][3])]
            child_index = depth_2_children[parent_id]
            depth_1_children_ret[depth_1_children_index[depth_1_parent_id]]['children'][child_index]['children'] = [returnChildren(paths[j][3])]
        ...
        for j in range(0, len(paths)):
            ...
            if depth_3_child_list is not None:
                ...
                if not in_iter:
                    ...
                    depth_1_children[depth_1_parent_id][0]['children'][z]['children'][y]['children'].append(returnChildren(paths[j][4]))
                    depth_2_child_index = depth_2_children[depth_2_parent_id]
                    child_index = depth_3_children[parent_id]
                    depth_1_children_ret[depth_1_children_index[depth_1_parent_id]]['children'][depth_2_child_index]['children'][child_index]['children'].append(returnChildren(paths[j][4]))
            else:
                if parent_id in depth_3_children.keys():
                    print(depth_3_children.keys())
                depth_1_children[depth_1_parent_id][0]['children'][z]['children'][y]['children'] = [returnChildren(paths[j][4])]
                depth_2_child_index = depth_2_children[depth_2_parent_id]
                child_index = depth_3_children[parent_id]
                depth_1_children_ret[depth_1_children_index[depth_1_parent_id]]['children'][depth_2_child_index]['children'][child_index]['children'] = [returnChildren(paths[j][4])]
